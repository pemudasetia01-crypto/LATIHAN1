...
if not st.session_state['logged_in']:
    ...
    if uploaded_file:
        ...
        peri_val = poly_meter.length # Kira Perimeter
        
        label_data = calculate_survey_labels(df, poly_meter, offset_val)
        
        # MASUKKAN DATA KE DALAM ATRIBUT GEOJSON UNTUK QGIS
        gdf_poly = gpd.GeoDataFrame({
            'ID': [st.session_state['current_user']],
            'Luas_m2': [round(area_val, 3)],
            'Perimeter': [round(peri_val, 3)],
            'Label': [f"L:{area_val:.2f} P:{peri_val:.2f}"]
        ...
        # 1. Sempadan
        if show_poly:
            folium.Polygon(locations=[[p[1], p[0]] for p in poly_wgs.exterior.coords], color="#00FFFF", weight=2, fill=True, fill_opacity=0.1, popup=f"Luas: {area_val:.3f} m²").add_to(m)
        ...
        # 4. Luas & Perimeter (Paparan Tengah)
        if show_area:
            area_html = f"""
            <div style="font-size: {text_size+2}pt; color: #00FF00; font-weight: bold; text-shadow: 3px 3px 6px black; text-align: center; width: 250px; transform: translate(-50%,-50%); line-height: 1.1;">
                {area_val:.3f} m²<br>
