...
if not st.session_state['logged_in']:
    ...
    if uploaded_file:
        ...
        # 1. Kira Luas dan Perimeter
        area_val = poly_meter.area
        peri_val = poly_meter.length
        
        label_data = calculate_survey_labels(df, poly_meter, offset_val)
        
        # 2. Simpan label dalam Atribut GeoJSON (Supaya kekal di QGIS)
        # Kita masukkan info Luas, Perimeter dan User ke dalam GeoDataFrame
        gdf_poly = gpd.GeoDataFrame({
            'User': [st.session_state['current_user']],
            'Luas_m2': [round(area_val, 3)],
            'Perimeter_m': [round(peri_val, 3)],
            'Label_Full': [f"Luas: {area_val:.2f} m2 | Peri: {peri_val:.2f} m"]
        ...
        with export_container:
            st.download_button(
                "📥 Export GeoJSON (QGIS)", 
                gdf_poly.to_json(), 
                f"lot_{st.session_state['current_user']}.geojson", 
                "application/json", 
        ...
        if show_area:
            ...
            <div style="font-size: {text_size+2}pt; color: #00FF00; font-weight: bold; text-shadow: 2px 2px 4px black; text-align: center; width: 200px; transform: translate(-50%,-50%);">
                {area_val:.3f} m²<br>
                <span style="font-size: {text_size}pt; color: #FFA500;">P: {peri_val:.3f} m</span>
            </div>"""
            folium.Marker([poly_wgs.centroid.y, poly_wgs.centroid.x], icon=folium.DivIcon(html=area_peri_html)).add_to(m)

        # Tambah Bearing & Jarak
        if show_labels:
            ...
            for i, data in enumerate(label_data):
                ...
                label_html = f'<div style="transform: translate(-50%, -50%) rotate({data["rotation"]}deg); text-align: center; white-space: nowrap; font-size: {text_size}pt; color: yellow; font-weight: bold; text-shadow: 2px 2px black;">{data["bearing"]}<br>{data["distance"]}</div>'
